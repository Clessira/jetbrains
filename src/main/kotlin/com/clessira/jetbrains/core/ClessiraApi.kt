...
class ClessiraHttpException(val status: Int, message: String) : IOException(message)

/**
 * Capability discovery + signing + transport for one request. Port of
 * `extension.ts requestClessira`: the capability file is read on every
 ...
 */
class ClessiraApi(private val transport: ApiTransport = ApiTransport()) {
