...
import com.clessira.jetbrains.core.ElapsedFormat
...
internal abstract class ClessiraWidgetBase(protected val project: Project) :
    ...
    override fun install(statusBar: StatusBar) {
        ...
        connection.subscribe(
            ClessiraAppService.STATE_TOPIC,
            ClessiraAppService.StateListener { refresh() },
        )
        connection.subscribe(
            ClessiraSettings.TOPIC,
            ClessiraSettings.Listener { refresh() },
        )
    }
...
internal class ConnectionWidget(project: Project) : ClessiraWidgetBase(project) {
    override fun ID(): String = WIDGET_ID

    override fun getText(): String = when (ClessiraAppService.instance.status) {
        ConnectionStatus.CONNECTED -> "✓ Clessira"
        ConnectionStatus.DISCONNECTED, ConnectionStatus.NEEDS_APP -> "⚠ Clessira"
        ConnectionStatus.CHECKING -> "… Clessira"
        ConnectionStatus.UNSUPPORTED_OS -> "Clessira"
    }

    override fun getTooltipText(): String = when (ClessiraAppService.instance.status) {
        ConnectionStatus.CONNECTED -> "Clessira: Connected, click for actions"
        ConnectionStatus.DISCONNECTED -> "Clessira: Disconnected, click for actions"
        ConnectionStatus.CHECKING -> "Clessira: Checking connection…, click for actions"
        ConnectionStatus.NEEDS_APP -> "Clessira: App not reachable, click for actions"
        ConnectionStatus.UNSUPPORTED_OS -> "Clessira requires macOS"
    }
    ...
    companion object {
        const val WIDGET_ID = "ClessiraConnection"
    }
}

internal class ActivityWidget(project: Project) : ClessiraWidgetBase(project) {
    override fun ID(): String = WIDGET_ID

    override fun getText(): String {
        val current = ClessiraAppService.instance.currentActivity ?: return ""
    ...
    override fun getTooltipText(): String? {
        val current = ClessiraAppService.instance.currentActivity ?: return null
        return "Clessira: ${current.activityName} — click to track new activity"
    }
...
internal class ActivityWidgetFactory : StatusBarWidgetFactory {
    ...
    override fun getDisplayName(): String = "Clessira Current Activity"
    override fun isAvailable(project: Project): Boolean = SystemInfo.isMac
