package com.clessira.nowdoing.settings
 ...
 * `nowdoing.*` configuration keys (same names, defaults and ranges).
 */
@Service(Service.Level.APP)
@State(
    name = "NowDoingSettings",
    storages = [Storage("nowdoing.xml")],
    category = SettingsCategory.TOOLS,
)
class NowDoingSettings : PersistentStateComponent<NowDoingSettings.State> {
    ...
    companion object {
        @JvmField
        val TOPIC: Topic<Listener> = Topic.create("NowDoing settings changed", Listener::class.java)

        val instance: NowDoingSettings
            get() = service()
    }
}
