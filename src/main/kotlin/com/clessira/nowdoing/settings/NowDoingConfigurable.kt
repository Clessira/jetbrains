...
class NowDoingConfigurable : BoundConfigurable("NowDoing") {

    private val state get() = NowDoingSettings.instance.state

    override fun createPanel(): DialogPanel = panel {
        group("Branch Notifications") {
            row {
                checkBox("Notify NowDoing when the Git branch changes")
            ...
            row("Debounce (ms):") {
                intTextField(range = 0..10_000)
                    ...
                    .columns(8)
                    .comment("Quiet window after a branch change before NowDoing is notified")
            }
