...
import com.clessira.nowdoing.core.ActivitySearchItem
...
object ActivityPickerPopup {
    ...
    fun show(project: Project?) {
        val app = NowDoingAppService.instance
    ...
    private fun buildAndShow(project: Project?) {
        ...
        val popup: JBPopup = JBPopupFactory.getInstance()
            ...
            .setTitle("NowDoing: Start Activity")
            .setFocusable(true)
                ...
                withContext(Dispatchers.EDT) {
                    ...
                    result.fold(
                        ...
                        onFailure = { e ->
                            ...
                            val message = "NowDoing search failed: ${e.message ?: e}"
                            NowDoingAppService.log.info(message)
                            NowDoingNotifier.error(message)
                        },
                    )
                }
            }
        ...
        fun accept() {
            ...
            app.scope.launch(Dispatchers.IO) {
                ...
                withContext(Dispatchers.EDT) {
                    result.fold(
                        onSuccess = { started ->
                            NowDoingNotifier.info(
                                if (started.created) {
                                    "NowDoing: Activity created and started: ${started.activityName}"
                                } else {
                                    "NowDoing: Activity started: ${started.activityName}"
                                },
                            )
                            popup.closeOk(null)
                        },
                        onFailure = { e ->
                            val message = "NowDoing start failed: ${e.message ?: e}"
                            NowDoingAppService.log.info(message)
                            NowDoingNotifier.error(message)
                            busy = false
