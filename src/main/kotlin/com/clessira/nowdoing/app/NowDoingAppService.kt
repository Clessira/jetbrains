...
import com.clessira.nowdoing.core.Capability
import com.clessira.nowdoing.core.CurrentActivityResult
import com.clessira.nowdoing.core.NowDoingApi
 ...
 * kept in `extension.ts NowDoingExtension`.
 */
@Service(Service.Level.APP)
class NowDoingAppService(val scope: CoroutineScope) {
    ...
    fun ensureStarted() {
        ...
        if (!SystemInfo.isMac) {
            log.info("NowDoing plugin is inactive: requires macOS")
            return
        }
        ApplicationManager.getApplication().messageBus.connect()
            .subscribe(NowDoingSettings.TOPIC, NowDoingSettings.Listener { restartPolling() })
    ...
    fun testConnection() {
        scope.launch(Dispatchers.IO) {
            try {
                ...
                NowDoingNotifier.info("NowDoing reachable (HTTP $httpStatus).")
            } catch (e: Exception) {
                setStatus(ConnectionStatus.DISCONNECTED)
                NowDoingNotifier.error("Could not reach NowDoing: ${e.message ?: e}")
            }
        }
    }
    ...
    fun checkConnection(notify: Boolean) {
        if (!SystemInfo.isMac) {
            ...
            if (notify) NowDoingNotifier.warning("NowDoing requires macOS.")
            return
        }
        try {
            Capability.read()
            ...
            if (notify) {
                NowDoingNotifier.warnAppNotReachable()
            } else {
        ...
        try {
            api.healthcheck()
            setStatus(ConnectionStatus.CONNECTED)
            if (notify) NowDoingNotifier.info("NowDoing: Connected.")
        } catch (e: Exception) {
            setStatus(ConnectionStatus.DISCONNECTED)
            if (notify) {
                NowDoingNotifier.errorConnectionFailed(e.message ?: e.toString())
            } else {
    ...
    fun setStatus(newStatus: ConnectionStatus) {
        ...
        if (previous == ConnectionStatus.CONNECTED &&
            ...
            NowDoingNotifier.warnConnectionLost()
        }
        publishStateChanged()
    }
    ...
    private fun startPolling() {
        ...
        pollJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                ...
                delay(NowDoingSettings.instance.currentPollSecondsClamped * 1000L)
            }
        }
    }
    ...
    companion object {
        @JvmField
        val STATE_TOPIC: Topic<StateListener> = Topic.create("NowDoing state changed", StateListener::class.java)

        private const val RENDER_TICK_MS = 30_000L

        val log: Logger = Logger.getInstance(NowDoingAppService::class.java)

        val instance: NowDoingAppService
            get() = service()
    }
}
