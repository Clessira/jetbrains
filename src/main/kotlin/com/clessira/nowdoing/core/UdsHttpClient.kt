...
class NowDoingTimeoutException(detail: String) : IOException("timeout: $detail")

/** Connect-phase failure; "connection refused" in the message marks it retryable. */
class NowDoingConnectException(message: String, cause: Throwable? = null) : IOException(message, cause)
...
class UdsHttpClient(private val timeoutMs: Long = DEFAULT_TIMEOUT_MS) {
        ...
        try {
            if (!channel.connect(UnixDomainSocketAddress.of(socketPath))) {
                ...
                if (!channel.finishConnect()) {
                    throw NowDoingTimeoutException("connect to $socketPath")
                }
            }
        } catch (e: ConnectException) {
            throw NowDoingConnectException("connection refused: ${e.message}", e)
        } catch (e: NowDoingTimeoutException) {
            throw e
        } catch (e: IOException) {
            ...
            throw NowDoingConnectException("cannot reach socket $socketPath: ${e.message}", e)
        }
    }
    ...
    private fun awaitSelect(selector: Selector, deadlineNanos: Long, phase: String) {
        ...
        if (remainingMs <= 0) throw NowDoingTimeoutException(phase)
        val ready = selector.select(remainingMs)
        selector.selectedKeys().clear()
        if (ready == 0 && System.nanoTime() >= deadlineNanos) {
            throw NowDoingTimeoutException(phase)
        }
    }
