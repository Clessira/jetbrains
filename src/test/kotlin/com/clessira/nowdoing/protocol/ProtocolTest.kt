package com.clessira.nowdoing.protocol

import com.clessira.nowdoing.core.ActivityStartBody
import com.clessira.nowdoing.core.ApiTransport
import com.clessira.nowdoing.core.Auth
import com.clessira.nowdoing.core.BranchChangeBody
import com.clessira.nowdoing.core.NowDoingApi
import com.clessira.nowdoing.core.NowDoingConnectException
import com.clessira.nowdoing.core.NowDoingHttpException
import com.clessira.nowdoing.core.NowDoingTimeoutException
import com.clessira.nowdoing.core.RetryPolicy
import com.clessira.nowdoing.core.UdsHttpClient
...
class ProtocolTest {
    ...
    private lateinit var api: NowDoingApi
    private val rawClient = UdsHttpClient()
    private lateinit var tempDir: Path

    @BeforeAll
    fun startServer() {
        tempDir = Files.createTempDirectory("nowdoing-protocol-test")
        ...
        api = NowDoingApi(ApiTransport(capabilityPath = { capabilityFile }))
    }
    ...
    fun `a signed POST with a JSON body verifies end-to-end`() {
        val status = api.postBranchChange(
            BranchChangeBody(repo = "NowDoingMac", repoPath = "/tmp/NowDoingMac", branch = "main"),
        )
    ...
    fun `non-2xx responses surface as typed HTTP errors with the server message`() {
        val brokenApi = run {
            ...
            NowDoingApi(ApiTransport(capabilityPath = { file }))
        }
        val e = assertThrows(NowDoingHttpException::class.java) { brokenApi.healthcheck() }
    ...
    fun `a server that never responds times out within the deadline`() {
        FakeUdsServer(token, respond = false).use { silent ->
            ...
            val e = assertThrows(NowDoingTimeoutException::class.java) {
    ...
    fun `a missing socket file fails without being classified retryable`() {
        val headers = signedHeaders("GET", "/healthcheck")
        val e = assertThrows(NowDoingConnectException::class.java) {
